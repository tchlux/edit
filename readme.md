...
`Esc <`/`Esc >` file start/end, `Esc g g` goto line, `Esc q` fill paragraph,
...
`Esc y` cycle paste,
`C-/` / `C-_` / `C-x u` undo, `C-x r` redo, `C-x 2` split below, `C-x 3` split right,
...
`C-c x` run a remembered shell command for the current file,
